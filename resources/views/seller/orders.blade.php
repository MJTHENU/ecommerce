...
<div class="main-body"> <div class="container-fluid"> @php $vipc=vipc(); @endphp @if($vipc['bkey']==0)
	@include('seller.partials.vipc') @else <div class="row mt-25"> <div class="col-lg-12"> <div class="card">
	...
	<div class="col-lg-6">
		...
		<div class="col-lg-6">
			...
			<button type="button" onClick="onExcelExport()" class="btn btn-theme mb0 btn-padding"><i class="fa
			fa-download"></i> {{ __('Excel') }}</button>
		...
		<div class="card-body">
			...
			<div class="group-button">
				<button id="orderstatus_0" type="button" onClick="onDataViewByStatus(0)" class="btn btn-theme
				orderstatus active">{{ __('All') }} (@php echo OrderCountForSeller(0); @endphp)</button>
				@foreach($order_status_list as $row)
				<button id="orderstatus_{{ $row->id }}" type="button" onClick="onDataViewByStatus({{ $row->id }})"
					class="btn btn-theme orderstatus">{{ $row->ostatus_name }} (@php echo OrderCountForSeller($row->id);
					@endphp)</button>
				@endforeach
				</div>
