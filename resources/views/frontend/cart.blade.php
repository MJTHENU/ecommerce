...
<meta property="og:url" content="{{ url()->current() }}" /> <meta property="og:image" content="{{
	...
	property="og:image:height" content="315" /> @if($gtext['fb_publish']==1) <meta name="fb:app_id" property="fb:app_id"
	content="{{ $gtext['fb_app_id'] }}" />
@endif
<meta name="twitter:card" content="summary_large_image">
@if($gtext['twitter_publish'] == 1)
<meta name="twitter:site" content="{{ $gtext['twitter_id'] }}"> <meta name="twitter:creator" content="{{
	$gtext['twitter_id'] }}"> @endif <meta name="twitter:url" content="{{ url()->current() }}"> <meta
	name="twitter:title" content="{{ $gtext['og_title'] }}"> <meta name="twitter:description"
	content="{{ $gtext['og_description'] }}">
<meta name="twitter:image" content="{{ asset('public/media/'.$gtext['og_image']) }}"> @endsection @section('header')
	@include('frontend.partials.header') @endsection @section('content') <main class="main"> <!-- Page Breadcrumb -->
...
<div class="container"> <div class="row align-items-center"> <div class="col-lg-6"> <nav aria-label="breadcrumb"> <ol
	...
	<li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('Home') }}</a></li> <li class="breadcrumb-item active"
		aria-current="page">{{ __('Cart') }}</li> </ol> </nav>
		</div> <div class="col-lg-6"> <div class="page-title"> <h1>{{ __('Cart') }}</h1> </div>
		</div> </div> </div> </div> <!-- /Page Breadcrumb/ -->

		<!-- Inner Section -->
<section class="inner-section inner-section-bg"> <div class="container"> @php
	$CartDataList=Cart::instance('shopping')->content(); $tax_rate=$gtax['percentage']; config(['cart.tax'=>
	$tax_rate]);
	@endphp
	@if(count($CartDataList)>0)
	<div class="row">
		<div class="col-xl-12"> <div class="table-responsive shopping-cart"> <table class="table">
			<thead>
			<tr>
			<th>{{ __('Image') }}</th>
			<th>{{ __('Product') }}</th>
		...
		<span class="pro-name"><a href="{{ route('frontend.product', [$row->id, str_slug($row->name)]) }}">{{ $row->name
				}}</a></span>
		</td>
		<td class="pro-store-w" data-title="{{ __('Sold By') }}:">
		<a href="{{ route('frontend.stores', [$row->options->seller_id, str_slug($row->options->store_name)]) }}">{{
			$row->options->store_name }}</a>
		</td>
		...
		<a data-id="{{ $row->rowId }}" id="removetoviewcart_{{ $row->id }}" onclick="onRemoveToCart({{ $row->id }})"
			href="javascript:void(0);" class="pro-remove"><i class="bi bi-x-lg"></i></a>
		</td>
		</tr>
		@endforeach
		</tbody>
	...
	</div> </div> <div class="row"> <div class="col-lg-7"></div> <div class="col-lg-5 mt10">
	...
	<div class="carttotals-body">
		...
		<tbody>
		<tr>
			<td><span class="title">{{ __('Price Total') }}</span><span class="price viewcart_price_total"></span></td>
		</tr>
		...
		<tr>
			<td><span class="total">{{ __('Total') }}</span><span class="total-price viewcart_total"></span></td>
		</tr>
		</tbody>
		</table>
	...
	<div class="col-sm-8 offset-sm-2 col-md-6 offset-md-3 col-lg-4 offset-lg-4 col-xl-4 offset-xl-4 col-xxl-4
		offset-xxl-4"> <div class="empty_card"> <div class="empty_img">
