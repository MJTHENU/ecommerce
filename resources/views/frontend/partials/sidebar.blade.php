...
<div class="sidebar">
	...
	<div class="widget-card">
		...
		<div class="widget-body">
			<ul class="widget-list">
				@php $SubCategoryListForFilter = SubCategoryListForFilter(); @endphp
				@foreach ($SubCategoryListForFilter as $row)
				<li>
					<div class="icon">
						<a href="{{ route('frontend.product-subcategory', [$row->id, $row->slug]) }}">
							<img src="{{ asset('public/media/'.$row->thumbnail) }}" alt="{{ $row->name }}" />
						</a>
					</div>
					<div class="desc">
						<a href="{{ route('frontend.product-subcategory', [$row->id, $row->slug]) }}">{{ $row->name }}</a>
					</div>
					<div class="count">{{ $row->TotalProduct }}</div>
				</li>
				@endforeach
