...
class ProductSubCategoryController extends Controller
{
    ...
    public function getProductSubCategoryPage($id, $title){
		
		$params = array('subcat_id' => $id);
		
		$mdata = Subcategory::where('id', '=', $id)->where('is_publish', '=', 1)->first();
		if($mdata !=''){
			$metadata = $mdata;
		}else{
			$metadata = array(
        ...
        return view('frontend.product-subcategory', compact('params', 'metadata', 'category_variation', 'datalist'));
    }
	
	//Get data for Product Category Pagination
	public function getProductSubCategoryGrid(Request $request){
		...
		if($request->ajax()){
			...
			return view('frontend.partials.product-subcategory-grid', compact('category_variation', 'datalist'))->render();
		}
	}
}
