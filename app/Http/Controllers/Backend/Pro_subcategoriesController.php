...
class Pro_subcategoriesController extends Controller
{
	...
	public function getProductSubCategoriesPageLoad()
	{
		...
		$datalist = DB::table('pro_categories')
			...
			->select('pro_categories.id', 'pro_categories.parent_id','pro_categories.name', 'pro_categories.thumbnail', 'pro_categories.lan', 'pro_categories.is_subheader', 'pro_categories.is_publish', 'tp_status.status', 'languages.language_name')
			->whereRaw('pro_categories.parent_id is NOT NULL')
			->orderBy('pro_categories.id','desc')
			->paginate(10);

		$parentcatdatalist = DB::table('pro_categories')
			->join('tp_status', 'pro_categories.is_publish', '=', 'tp_status.id')
			->join('languages', 'pro_categories.lan', '=', 'languages.language_code')
			->select('pro_categories.id', 'pro_categories.parent_id','pro_categories.name', 'pro_categories.thumbnail', 'pro_categories.lan', 'pro_categories.is_subheader', 'pro_categories.is_publish', 'tp_status.status', 'languages.language_name')
			->orderBy('pro_categories.id', 'desc')
		...
		return view('backend.product-subcategories', compact('media_datalist', 'statuslist', 'languageslist', 'datalist', 'parentcatdatalist'));
	}
	...
	public function getProductSubCategoriesTableData(Request $request)
	{
		...
		if ($request->ajax()) {
			...
			return view('backend.partials.product_subcategories_table', compact('datalist'))->render();
		}
	}
	...
	public function saveProductSubCategoriesData(Request $request)
	{
		...
		$parentcat = esc($request->input('parentcat'));
