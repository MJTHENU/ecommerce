...
function getTax($tax_pos = 0)
{

	$results = Tax::offset($tax_pos)->limit(1)->get();
...
{
	$datalist = Product::where('id', $product_id)->first();
	return $datalist;
}

function getTaxDetails($tax_id)
{
	$taxlist = Tax::where('id', $tax_id)->first();
	return $taxlist;
}
