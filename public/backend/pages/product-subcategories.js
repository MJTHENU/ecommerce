...
$(function () {
	...
	resetForm("SubDataEntry_formId");
	
	$("#submit-form").on("click", function () {
        $("#SubDataEntry_formId").submit();
    });
	...
	$("#parentcat").val(0).trigger("chosen:updated");
...
function onPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/backend/getProductSubCategoriesTableData?page="+page
		+"&search="+$("#search").val()
...
function onRefreshData() {
	$.ajax({
		url:base_url + "/backend/getProductSubCategoriesTableData?search="+$("#search").val()
		+"&language_code="+$('#language_code').val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
...
function resetForm(id) {
	...
	$("#parentcat").trigger("chosen:updated");
...
jQuery('#SubDataEntry_formId').parsley({
    listeners: {
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/saveProductSubCategoriesData',
		data: $('#SubDataEntry_formId').serialize(),
...
function onLoadEditData() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/getProductSubCategoriesById',
		data: 'id='+RecordId,
		success: function (response) {
			var data = response;
			...
			$("#parentcat").val(data.parent_id).trigger("chosen:updated");
...
function onConfirmDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/deleteProductSubCategories',
		data: 'id='+RecordId,
		success: function (response) {
			var msgType = response.msgType;
			var msg = response.msg;
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/bulkActionProductSubCategories',
...
function onCategorySlug() {
	...
	if(strLength>0){
		$.ajax({
			type : 'POST',
			url: base_url + '/backend/hasProductSubCategorySlug',
			data: 'slug='+StrName,
			success: function (response) {
